...
| **② 命令行工具** | 本地 CLI（`rbcp`） | 链接/清单 → 本地 Markdown，可脚本化、给 AI Agent 编排 | 命令行党、自动化 |
| **③ 本地服务 + 网页端** | `rbcp serve` 起的本地网页 | 浏览器（含手机）点点点：提交、看进度、读 Markdown、看用量费用 | 想要图形界面的人 |

> 为什么抓清单单独做成油猴：下载和落知识库**必须**本地进程（浏览器写不了本地目录、跑不了 ffmpeg），而抓清单**只有浏览器**能在你登录态里安全做（避风控）。所以 ① 负责浏览器才能干的那段，②③ 是同一套本地核心的两层皮。三者配合，不是三选一。

---

## 路线图（Roadmap）

> 当前稳定版聚焦"链接 → 忠实原文"。下一步（**0.6，设计中**）把重心移到**帮人快速读懂**。

- **0.6 · 速览产品**：在忠实原文之上加一层"速览"——重点**高亮跳读** / **卡片金句** / **脉络梳理**，目标一竖屏抓住重点（随时可切回看原文）。
- 形态上引入 **RBCP Desktop**（跨平台桌面客户端，Mac/Windows），命令行 `rbcp` 保留并增强。
- 架构按 **Extract**（采集转录→忠实原文）→ **Digest**（提炼高亮/金句/脉络）→ **Render**（呈现）分层；引擎保持开源。
- 详见 [0.6 速览产品 devlog](docs/devlog/2026-06-15-0.6-speed-read-product.md) 与 [PLAN.md](PLAN.md) §v0.6。设计进行中，欢迎在 issue 区讨论。
