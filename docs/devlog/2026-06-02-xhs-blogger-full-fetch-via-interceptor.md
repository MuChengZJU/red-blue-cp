...
> ⚠️ **实现已演进**：本文记录的是**一次性手动抓取**用的 XHR 注入拦截器（当时用 gstack 浏览器，可行）。
> 但**产品化代码没用这套**——pydoll 里页面会绕过 JS 注入的拦截器，改成了 **pydoll 原生网络捕获**
> （见 [pydoll 原生捕获复盘](2026-06-03-pydoll-native-capture-and-login.md)）。
> 本文的**平台经验仍然有效**（别滚 DOM、编号会乱、按内容核对）。小红书接口的完整事实速查见
> [小红书接口速查](../reference/xhs-api-notes.md)。
