...
- [ ] 主页面整合「单条 / 批量」两个标签，去掉独立 `/batches` 页（路由重定向回主页）。
- [ ] 批次进任务列表：每批一个标题/名字（可自定义，缺省自动生成），默认显示前几条（可滚动），可展开全部。
- [ ] **批量逐条建任务**：`run_batch` 每条建 job（`batch_item` 加 `job_id` 列关联），批次条目可点进 `/jobs/{id}` 详情，用量账单/重试全部复用任务体系。
- [ ] 导入按钮/下拉框重做（现在粗糙）。
- [ ] **去重检测**：`dedup_key(url)`（B 站 BV 号 / 小红书 note_id 归一，token 参数不同也算同内容）。单条提交已成功下过 → 409 + 已有任务信息，前端弹「已下过，是否重下」，force 才重下；批量导入默认跳过已下过的、报跳过数，可勾选强制全量。
- [ ] 顺手：`rbcp serve` 加 `--port`（现在写死 8000，多会话规则里"--port 错开"才成立）。
