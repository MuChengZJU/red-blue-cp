...
3. **博主全量**：列清单 → 过滤 → 确认 → 下载
   - 拉取实现（小红书）：pydoll 驱动系统 Chrome，拦截 `user_posted` 接口（见 SPEC §拉取实现 / [设计](../docs/devlog/2026-06-02-blogger-full-and-comments-design.md)）。B 站另一套机制，本期不做。
...
```
P1a · 批量 + 限流              (1 天)
P1b · 博主全量（pydoll 拦截器） (1.5 天)
P1c · 评论提取（同套浏览器代签）(0.5 天)
